...
sed -r -i "s|$pattern|$replace|g" ${ISTIO_DIR}/central/service-entries.yaml
...
kubectl apply -n hipster2  -f ${ISTIO_DIR}/central
...
sed -r -i "s|$pattern|$replace|g" ${ISTIO_DIR}/remote/service-entries.yaml
...
# Deploy part of hipster app on central cluster in the namespace hipster2
kubectl apply -n hipster1  -f ${ISTIO_DIR}/remote
